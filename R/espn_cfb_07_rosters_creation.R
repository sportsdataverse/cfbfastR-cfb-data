...
reshape_rosters <- function(g) flat_block_df(g$game_rosters, g)

build_rosters_season <- function(season, master = fetch_master_local(), live = TRUE, publish = TRUE) {
  build_season(season, "rosters", "rosters", "espn_cfb_rosters", reshape_rosters,
...
main <- function() {
  ...
  for (y in opt$start_year:opt$end_year) build_rosters_season(y, master = master, live = TRUE)
}
