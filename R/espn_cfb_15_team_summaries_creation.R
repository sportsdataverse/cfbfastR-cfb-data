...
# explosive / drive metrics per team, passing/rushing/receiving leaders, and per-metric
# percentiles. Unlike the per-game reshape_* creators this is a season-level aggregation,
# so it writes + publishes the resulting season tables through the shared dataset helpers
# (write_dataset / publish_dataset) instead of build_season().

fmt_elapsed <- function(start) {
    sprintf("%.1fs", as.numeric(difftime(Sys.time(), start, units = "secs")))
}
...
summarize_passer_df <- function(x, by) {
    tmp <- x |>
        ...
        summarize(
            ...
            passer_player_name = dplyr::first(passer_player_name, na_rm = TRUE),
            plays = n(),
...
summarize_receiver_df <- function(x, by) {
    tmp <- x |>
        # filter(
        ...
        # ) |>
        summarize(
            receiver_player_name = dplyr::first(receiver_player_name, na_rm = TRUE),
...
summarize_rusher_df <- function(x, by) {
    tmp <- x |>
        # filter(
        ...
        summarize(
            ...
            rusher_player_name = dplyr::first(rusher_player_name, na_rm = TRUE),
            plays = n(),
            games = n_distinct(game_id),
            team_games = dplyr::last(team_games),
            playsgame = plays / games,
...
summarize_team_df <- function(x, ascending = FALSE, remove_cols = c()) {
    tmp <- x |>
        summarize(
            plays = n(),
            playsgame = plays / n_distinct(game_id),
            ...
            TEPA = sum(EPA),
            EPAplay = mean(EPA),
            EPAdrive = TEPA / n_distinct(drive_id),
            EPAgame = TEPA / n_distinct(game_id),

            # yards
            yards = sum(yards_gained),
            yardsplay = mean(yards_gained),
            yardsgame = yards / n_distinct(game_id),
            play_stuffed = mean(play_stuffed),

            # drives
            drives = n_distinct(drive_id),
            drivesgame = drives / n_distinct(game_id),
            ...
            early_down_EPA = mean(early_down_EPA, na.rm = TRUE),

            # Field Position
            start_position = mean(drive_start_yards_to_goal),

            # Profile stuff
            nonExplosiveEpaPerPlay = mean(nonExplosiveEpa, na.rm = TRUE),
            line_yards = mean(line_yards, na.rm = TRUE),
            opportunity_rate = mean(opportunity_run, na.rm = TRUE),

            # Available Yards Pct
            # available_yards_pct = mean(available_yards_pct, na.rm = TRUE)
        )
    if (ascending) {
        tmp <- tmp |>
            mutate(
    ...
    tmp |> select(!matches(paste(remove_cols, sep = "|")))
}
...
clean_columns <- function(x) {
    ...
    x |> rename_with(~new_columns, all_of(old_columns))
}
...
prepare_for_write <- function(x, yr, schools) {
    ...
    tmp <- x |>
        ...
        dplyr::left_join(schools, by = join_by(pos_team_id)) |>
        select(
            team_id = pos_team_id,
            pos_team,
...
prepare_percentiles <- function(x) {
    tmp <- x |>
        summarize(
            GEI = dplyr::first(GEI, na_rm = TRUE),
            EPAplay = mean(EPA, na.rm = TRUE),
            pass_success = mean(epa_success * pass, na.rm = TRUE),
            rush_success = mean(epa_success * rush, na.rm = TRUE),
            early_down_success = mean(early_down_success, na.rm = TRUE),
            early_down_EPA = mean(early_down_EPA, na.rm = TRUE),
            late_down_success = mean(late_down_success, na.rm = TRUE),
            ...
            yardsrush = mean(yds_rushed, na.rm = TRUE),
            lineyards = mean(line_yards, na.rm = TRUE),
            opportunity_run = mean(opportunity_run, na.rm = TRUE),
            third_down_distance = mean(third_down_distance, na.rm = TRUE),
            .by = c(game_id, pos_team)
        )

    summ_tmp <- tmp |> reframe(
        pctile = seq(.01, .99, by = .01),
        dplyr::across(
            -c(pctile, game_id, pos_team),
            ~ quantile(.x, probs = pctile, na.rm = TRUE),
            .names = "{.col}"
        )
    )
    summ_tmp
...
adjust_epa <- function(plays) {
    ...
    ridge_model <- glmnet::glmnet(x, y, alpha = 0, lambda = target_lambda, intercept = TRUE)
    ...
    off_epa_game <- pbp.base |>
        summarise(
            ...
            rawOffEPA = mean(EPA, na.rm = TRUE),
            .by = c(game_id, pos_team_id, def_pos_team_id)
        ) |>
        left_join(defense, by = join_by(def_pos_team_id == team_id)) |>
    ...
    def_epa_game <- pbp.base |>
        summarise(
            ...
            pos_team = dplyr::last(pos_team),
            def_pos_team = dplyr::last(def_pos_team),
            rawDefEPA = mean(EPA, na.rm = TRUE),
            .by = c(game_id, def_pos_team_id, pos_team_id)
        ) |>
        left_join(offense, by = join_by(pos_team_id == team_id)) |>
    ...
    opp.adj <- off_epa_game |>
        left_join(def_epa_game, by = join_by(game_id, pos_team_id == def_pos_team_id))
    ...
    team.adj <- opp.adj |>
        ...
        dplyr::summarise(
            pos_team = dplyr::last(pos_team),
            team_games = dplyr::n(),
            valid_games = sum(!is.na(adjOffEPA) & !is.na(adjDefEPA), na.rm = TRUE),
            dplyr::across(dplyr::contains("EPA") | dplyr::contains("strength"), ~ mean(.x, na.rm = TRUE))
        ) |>
        dplyr::ungroup() |>
        dplyr::mutate(
...
build_team_summaries_season <- function(yr, publish = TRUE) {
    cli::cli_h1("Processing {yr} season")
    season_start <- Sys.time()
    cli::cli_alert_info("Downloading plays...")
    plays <- cfbfastR::load_cfb_pbp(seasons = c(yr))
    cli::cli_alert_info("Downloading schedule...")
    schedules <- cfbfastR::load_cfb_schedules(seasons = c(yr))

    # cli::cli_alert_info("Downloading teams...")

    cli::cli_alert_info("Found {nrow(plays)} total plays, filtering to FBS/FBS")

    plays <- plays |>
        dplyr::left_join(schedules |> dplyr::select(game_id, neutral_site, home_id, home_division, away_id, away_division), by = join_by(game_id)) |>
        filter(
            ((pass == 1) | (rush == 1)) &
        ...
        mutate(
            kneel_down = dplyr::case_when(
                (pass == 1) ~ FALSE,
                stringr::str_detect(play_text, "\\s?kneel-down\\s?") ~ TRUE,
                stringr::str_detect(play_text, "\\s?kneel down\\s?") ~ TRUE,
                stringr::str_detect(play_text, "\\s?kneel\\s?") ~ TRUE,
                stringr::str_detect(play_text, "\\s?takes a knee\\s?") ~ TRUE,
                    ...
                    (
                        ...
                        (
                            stringr::str_detect(play_text, "^team run for a loss of 1 yard") |
                                stringr::str_detect(play_text, "^team run for a loss of 2 yards")
                        )
                ) ~ TRUE,
                .default = FALSE
            )
        ) |>
        dplyr::filter(kneel_down == FALSE) |>
        cfbfastR::clean_play_text() |>
        mutate(
            ...
            opp_highlight_yards = dplyr::case_when(
                (opportunity_run == TRUE) ~ highlight_yards,
                (opportunity_run == FALSE) & (rush == 1) ~ 0,
                .default = NA_real_
            ),
            nonExplosiveEpa = dplyr::case_when(
                !is.na(EPA) & (explosive == FALSE) ~ EPA,
                .default = NA_real_
            ),
        ) |>
    ...
    cli::cli_alert_info("Found {nrow(plays)} total FBS/FBS non-garbage-time plays, summarizing offensive data [{fmt_elapsed(season_start)}]")
    ...
    team_qb_data <- team_qb_data |>
        dplyr::left_join(team_qb_ranks, by = join_by(pos_team_id, passer_player_id))
    ...
    team_rb_data <- plays |>
        ...
        summarize_rusher_df(by = c(pos_team_id, rush_player_id))
    ...
    team_rb_data <- team_rb_data |>
        dplyr::left_join(team_rb_ranks, by = join_by(pos_team_id, rush_player_id))
    ...
    team_wr_data <- plays |>
        ...
        summarize_receiver_df(by = c(pos_team_id, receiver_player_id))
    ...
    team_wr_data <- team_wr_data |>
        dplyr::left_join(team_wr_ranks, by = join_by(pos_team_id, receiver_player_id))
    ...
    cli::cli_alert_info("Summarizing defensive data [{fmt_elapsed(season_start)}]")
    ...
    cli::cli_alert_info("Merging offensive and defensive data, calculating full season ranks [{fmt_elapsed(season_start)}]")

    team_overall_data <- left_join(team_off_data, team_def_data, by = join_by(pos_team_id == def_pos_team_id), suffix = c("_off", "_def"))
    team_drives_data <- left_join(team_off_drives_data, team_def_drives_data, by = join_by(pos_team_id == def_pos_team_id), suffix = c("_off", "_def"))
    team_pass_data <- left_join(team_off_pass_data, team_def_pass_data, by = join_by(pos_team_id == def_pos_team_id), suffix = c("_off", "_def"))
    team_rush_data <- left_join(team_off_rush_data, team_def_rush_data, by = join_by(pos_team_id == def_pos_team_id), suffix = c("_off", "_def"))
    ...
    team_data <- left_join(team_overall_data, team_drives_data, by = join_by(pos_team_id), suffix = c("", "_drive"))
    team_data <- left_join(team_data, team_pass_data, by = join_by(pos_team_id), suffix = c("", "_pass"))
    team_data <- left_join(team_data, team_rush_data, by = join_by(pos_team_id), suffix = c("", "_rush"))

    cli::cli_alert_info("running ridge regression adjustments.... [{fmt_elapsed(season_start)}]")
    adj_EPA_df <- adjust_epa(plays)

    cli::cli_alert_info("Generating year and team CSVs...")
    ...
    schools <- plays |>
        ...
        dplyr::distinct(pos_team_id, .keep_all = TRUE) |>
    ...
    team_data <- team_data |>
        ...
        dplyr::left_join(adj_EPA_df |> dplyr::select(-pos_team), by = join_by(team_id))
    ...
    cli::cli_alert_info("Writing + publishing season datasets [{fmt_elapsed(season_start)}]")
    datasets <- list(
        list(df = team_off_pctls, dataset = "percentiles", stem = "cfb_percentiles", tag = "espn_cfb_percentiles"),
        list(df = team_data, dataset = "team_summaries", stem = "cfb_team_summaries", tag = "espn_cfb_team_summaries"),
        list(df = team_qb_data, dataset = "passing", stem = "cfb_passing", tag = "espn_cfb_passing"),
        list(df = team_rb_data, dataset = "rushing", stem = "cfb_rushing", tag = "espn_cfb_rushing"),
        list(df = team_wr_data, dataset = "receiving", stem = "cfb_receiving", tag = "espn_cfb_receiving")
    )
    for (d in datasets) {
        write_dataset(d$df, d$dataset, yr, d$stem)
        if (publish) publish_dataset(d$dataset, yr, d$stem, d$tag)
    }

    cli::cli_alert_success("Finished {yr}: wrote {nrow(team_data)} teams in {fmt_elapsed(season_start)}")
}
...
main <- function() {
    ...
    seasons <- opt$start_year:opt$end_year
    purrr::walk(seasons, build_team_summaries_season)
    cli::cli_alert_success("All {length(seasons)} season{?s} complete in {fmt_elapsed(script_start)}")
}
