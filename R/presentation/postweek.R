...
library(readr)
library(stringr)
if (!exists("read_team_summaries")) source("R/presentation/_summary_paths.R")
...
generate_adj_epa_comp <- function(classif) {
    ...
    x_margin <- sum(abs(range(base$adjOffEPA, na.rm = TRUE))) * 0.10
    y_margin <- sum(abs(range(base$adjDefEPA, na.rm = TRUE))) * 0.05

    base |>
        ggplot2::ggplot(ggplot2::aes(x = adjOffEPA, y = adjDefEPA, team = pos_team)) +
        ggplot2::annotate(
            "text",
            x = max(base$adjOffEPA, na.rm = TRUE) - x_margin,
            y = max(base$adjDefEPA, na.rm = TRUE) - y_margin,
            size = 4,
        ...
        ggplot2::annotate(
            "text",
            x = min(base$adjOffEPA, na.rm = TRUE) + x_margin,
            y = min(base$adjDefEPA, na.rm = TRUE) + y_margin,
            size = 4,
        ...
        ggplot2::geom_vline(xintercept = mean(base$adjOffEPA, na.rm = TRUE), linetype = "dashed", color = "red") +
        ggplot2::geom_hline(yintercept = mean(base$adjDefEPA, na.rm = TRUE), linetype = "dashed", color = "red") +
...
def_stats_graphic <- team.adj |>
    ...
    gt::fmt_markdown(c(pos_team)) |>
    gtExtras::gt_hulk_col_numeric(adjDefEPA, domain = team.adj$adjDefEPA, reverse = TRUE) |>
    gtExtras::gt_hulk_col_numeric(EPAplay_def, domain = team.adj$EPAplay_def, reverse = TRUE) |>
    gtExtras::gt_hulk_col_numeric(success_def, domain = team.adj$success_def, reverse = TRUE) |>
    gtExtras::gt_hulk_col_numeric(yardsplay_def, domain = team.adj$yardsplay_def, reverse = TRUE) |>
    gt::cols_label(
        "pos_team" = "Team",
...
valid_fbs_teams <- cfbfastR::cfbd_team_info(only_fbs = TRUE)
...
games <- games_raw |>
    ...
    dplyr::left_join(current_week, by = join_by(season, season_type == season_type_key, week)) |>
    dplyr::filter(
...
passer <- week_pbp |>
    ...
    dplyr::summarize(
        ...
        Pass_TD = sum(pass_td, na.rm = TRUE),
        Pass_INT = sum(int, na.rm = TRUE),
        sacks = sum(sack_vec, na.rm = TRUE),
        sack_adj_yards = yards - sum(yds_sacked, na.rm = TRUE),
        yards_db = sack_adj_yards / dropbacks,
        EPA_db = mean(EPA, na.rm = TRUE),
...
pass_epa_graphic <- passer |>
    ...
    gtExtras::gt_merge_stack(passer_player_name, pos_team, small_cap = FALSE, font_size = c("18px", "12px"), font_weight = c("bold", "normal"), palette = c("black", "black")) |>
...
rusher <- week_pbp |>
    ...
    dplyr::summarize(
        ...
        carries = dplyr::n(),
        # attempts = sum(rush, na.rm = TRUE),
        yards = sum(yds_rushed, na.rm = TRUE),
        rushing_td = sum(rush_td, na.rm = TRUE),
        fumbles = sum(fumble_vec, na.rm = TRUE),
        yards_per_carry = mean(yds_rushed, na.rm = TRUE),
...
rushing_epa_graphic <- rusher |>
    ...
    gt::fmt_markdown(c(pos_team, def_pos_team_name)) |>
    gtExtras::gt_merge_stack(rusher_player_name, pos_team, small_cap = FALSE, font_size = c("18px", "12px"), font_weight = c("bold", "normal"), palette = c("black", "black")) |>
...
receiver <- week_pbp |>
    ...
    dplyr::summarize(
        ...
        yards = sum(yds_receiving, na.rm = TRUE),
        yards_per_target = mean(yds_receiving, na.rm = TRUE),
        yards_per_rec = dplyr::if_else(receptions == 0, 0, yards / receptions),
        rec_td = sum(pass_td, na.rm = TRUE),
        fumbles = sum(fumble_vec, na.rm = TRUE),
        EPAplay = mean(EPA, na.rm = TRUE),
        success = mean(EPA > 0, na.rm = TRUE)
    ) |>
...
rec_epa_graphic <- receiver |>
    ...
    gt::fmt_markdown(c(pos_team, def_pos_team_name)) |>
    gtExtras::gt_merge_stack(receiver_player_name, pos_team, small_cap = FALSE, font_size = c("18px", "12px"), font_weight = c("bold", "normal"), palette = c("black", "black")) |>
...
for (i in seq_along(thread_content)) {
    g <- thread_content[[i]]
    reply <- fire_skeet(g, reply = NULL, is_live_run) # turning off threading
    cli::cli_alert_info("Sleeping for {delay} seconds before skeeting again...")
    Sys.sleep(delay)
}
