...
cli::cli_h1("CFB game previews + skeets")
...
games <- purrr::map(seq_len(nrow(current_week)), function(i) {
    cur <- current_week[i, ]
...
if (!("notes" %in% colnames(games))) {
    games_cfbd <- purrr::map(seq_len(nrow(current_week)), function(i) {
        cur <- current_week[i, ]
    ...
    games <- games |>
        dplyr::left_join(games_cfbd |> dplyr::select(game_id, notes), by = join_by(game_id))
}
...
rankings <- rankings_raw |>
    ...
    dplyr::left_join(teams |> dplyr::select(team_id, school), by = join_by(school)) |>
    dplyr::mutate(
...
selected_games <- games |>
    ...
    # ) |>
    # dplyr::left_join(tv_windows, by = join_by(game_day == day), relationship = "many-to-many") |>
    # dplyr::mutate(
    #     in_current_window = current_window & (game_time_slot >= start_hour) & (game_time_slot < end_hour) & (game_day == current_day)
    # ) |>
    # dplyr::group_by(game_id) |>
    # dplyr::slice_max(order_by = in_current_window, n = 1, with_ties = FALSE) |>
    # dplyr::ungroup() |>
    dplyr::filter(
        (
            (
    ...
    dplyr::left_join(rankings |> dplyr::select(home_team_rank = rank, team_id), by = join_by(home_team_id == team_id)) |>
    dplyr::left_join(rankings |> dplyr::select(away_team_rank = rank, team_id), by = join_by(away_team_id == team_id)) |>
    dplyr::slice_min(order_by = dplyr::row_number(), n = 1, with_ties = FALSE, .by = game_id) |>
...
generate_matchup_df <- function(home_id, away_id, season_override) {
    season <- if (is.null(season_override)) current_week$season[1] else season_override
    season_data <- read_team_summaries(season)

    home_data <- season_data |>
        dplyr::filter(team_id == home_id) |>
        clean_team_data()

    away_data <- season_data |>
        dplyr::filter(team_id == away_id) |>
        clean_team_data()
    ...
    matchup_df <- away_off_home_df |>
        dplyr::left_join(away_def_home_off, by = join_by(away_team, away_logo_url, home_team, home_logo_url, name, is_rank))
    ...
    final <- values |>
        dplyr::left_join(ranks, by = join_by(name, away_team, away_logo_url, home_team, home_logo_url)) |>
...
skeet <- function(text, images, images_alt, video, video_alt, langs,
    ...
    if (!missing(images)) {
        ...
        if (!missing(images_alt)) {
            img_incl <- lapply(seq_along(blob), function(i) {
                list(
                    image = blob[[i]]$blob, alt = images_alt[[i]],
    ...
    req <- httr2::req_body_json(httr2::req_auth_bearer_token(httr2::request("https://bsky.social/xrpc/com.atproto.repo.createRecord"), token = auth$accessJwt), data = list(
        repo = auth$did,
...
fire_skeet <- function(row, reply = NULL, live_run = FALSE) {
    cli::cli_alert_info("Generating skeet for game {row$game_id} (reply: {!is.null(reply)}, live_run: {live_run})")

    cli::cli_alert_info("Generating skeet image data...")
    ...
    cli::cli_alert_info("Generating skeet content...")
...
if (nrow(selected_games) > 0) {
    cli::cli_alert_info("Skeeting for {nrow(selected_games)} relevant FBS game{?s} (live_run: {is_live_run})")
    for (i in seq_len(nrow(selected_games))) {
        g <- selected_games[i, ]
        reply <- fire_skeet(g, NULL, is_live_run) # disable threading
        cli::cli_alert_info("Waiting {delay} seconds before skeeting again...")
        Sys.sleep(delay)
    }
} else {
    cli::cli_alert_warning("No relevant FBS games, bailing out")
}
