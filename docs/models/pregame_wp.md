...
Metrics are pooled **leave-one-season-out (LOSO)** out-of-fold predictions over 2005-2025, so they are out-of-**season**. They are NOT out-of-**application**: the features are still same-game, so these are explanatory-fit numbers, not forecasting numbers. **Measured 2026-09-07, the honest pregame figures are R² 0.259 and Brier 0.2036** — a **51.6% fall in explained variance** against this page's 0.535, and a **19.9% relative rise in Brier** against 0.1698 (7,915 games, 2015-2025, each team rated only on its recent-average 5FR from prior games). The two magnitudes differ and should be quoted separately: the R² roughly halves, the Brier does not. The gap survives re-optimising the Gaussian σ — sweeping it, the best achievable as-of Brier is still 0.2036 — so it is the feature, not the calibration. *(Internal control: refitting the same-game relation as a plain linear map on the same 2015-2025 rows the as-of arm uses gives R² 0.565, so the as-of arm is not being compared against a differently-specified baseline. 0.535 above remains the canonical published number — the 10-tree XGBoost LOSO fit over 37,774 team-games — and is what the 51.6% is measured against.)* Quote 0.259 / 0.2036 for pregame use and the figures below for how well the index explains a game it can already see. The single `5FRDiff` feature recovers **PtsDiff R² 0.535** — one composite rating explains ~54% of point-margin variance — and the Gaussian-transformed win probability is well-calibrated: **WP weighted calibration error 0.0115**, **Brier 0.1698** against a **0.500 win base rate**. The single-panel calibration figure bins predicted pregame WP into 0.05 buckets and plots it against the empirical win rate (point size = n, y=x reference). In application the model is fed a team's *recent-average* 5FR to forecast a future opponent; the fitted object itself is the same-game `5FRDiff` → `PtsDiff` relationship.
