...
| matchup scoring_opp (drive scoring-opportunity glm) | `cfb_matchup/artifacts/scoring_opp_coef.json` (+ `_meta.json`) | bundled in-repo; consumed by `cfb_matchup_features` / `cfb_matchup_line` (stages 41/42) | 262,065 first-play-of-drive rows starting beyond the 40, the source pipeline's 2014–2023 drive frame | `python -m cfb_model_build.cfb_matchup train-scoring-opp` (stage 35) | refit on the shipped training rows reproduces every coefficient ≤ 1e-6 rel (`tests/cfb_matchup/test_glm_parity.py`, integration); applier ≤ 1e-9 vs R `predict.glm` on 36,151 drives of 2025 | 2026-09-15 (imported R fit) | frozen (retrain on demand via stage 35 + `--force`) |
| matchup rush_expect (P(rush) glm) | `cfb_matchup/artifacts/rush_expect_coef.json` (+ `_meta.json`; `score_diff` aliased) | bundled in-repo; consumed by stages 41/42 (`rroe = rush − P(rush)`) | 1,304,773 rush/pass plays, the source pipeline's corpus | `python -m cfb_model_build.cfb_matchup train-rush-expect` (stage 35) | refit reproduces every non-aliased coefficient ≤ 1e-6 rel on the shipped rows; applier ≤ 1e-9 vs R on 283,172 plays of 2025 | 2026-09-15 (imported R fit) | frozen (retrain on demand; era drift in rush rate is a known caveat, see meta) |
| matchup wepa_weights (120 situational EPA weights) | `cfb_matchup/artifacts/wepa_weights.json` (+ `_meta.json`) | bundled in-repo; consumed by stages 41/42 (`apply_wepa`) | random search over 500 candidate weight vectors scored in-sample by `lm(home MOV ~ home/away off/def WEPA)` adj-R² on the source's 2014–2023 games | TODO — trainer not yet ported (`wepa_weight_training.R`); applier gated: WEPA ≤ 1e-9 vs R on 283,172 plays of 2025 | applier parity only; no out-of-sample score yet (recorded follow-up) | 2024-06-02 (source fit) | frozen |
