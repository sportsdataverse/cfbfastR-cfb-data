...
import collections, json, os, re, unicodedata, urllib.request
import polars as pl
...
def _key() -> str:
    ...
    if key:
        return key
    for path in (os.path.expanduser("~/.Renviron"), os.path.expanduser("~/Documents/.Renviron")):
...
ALIASES = {
    "San José State": "San Jose State", "UConn": "Connecticut",
    "UL Monroe": "Louisiana Monroe", "Sam Houston": "Sam Houston State",
    "Jacksonville State": "Jacksonville State", "Massachusetts": "UMass",
    "UT San Antonio": "UTSA", "Hawai'i": "Hawai'i", "Miami": "Miami",
}
NAME = r"([A-Z][A-Za-z'\-\.]+(?: [A-Z][A-Za-z'\-\.]+)+) \([A-Z0-9&\-]{2,8}\)"

def production_2003() -> dict[tuple[str, str], float]:
    prod: dict[tuple[str, str], float] = collections.defaultdict(float)
    for stype, weeks in (("regular", range(1, 17)), ("postseason", range(1, 3))):
        for wk in weeks:
            try:
                plays = _get(f"https://api.collegefootballdata.com/plays?year=2003&week={wk}&seasonType={stype}")
            except Exception as exc:
                print(f"  wk {stype}/{wk}: {exc}"); continue
            for p in plays:
                ...
                if m:
                    prod[(off, norm(m.group(1)))] += yds; continue
...
def main() -> None:
    ...
    for loc, fn, aid in r.select("team_location", "full_name", "athlete_id").iter_rows():
    ...
    df = pl.DataFrame(rows, schema={
        "season": pl.Int64, "team_id": pl.Utf8, "player_id": pl.Utf8,
        "player_name": pl.Utf8, "unit": pl.Utf8, "prod_weight": pl.Float64, "position": pl.Utf8})
    ...
    keep = (df.group_by("team_id").agg(pl.col("prod_weight").sum().alias("y"))
              .filter(pl.col("y") >= MIN_TEAM_YARDS)["team_id"].to_list())
    df = df.filter(pl.col("team_id").is_in(keep))
    print(f"rows: {df.height}  resolved to ESPN athlete_id: {matched} ({100*matched/max(df.height,1):.1f}%)")
    print(f"schools unmapped: {sorted(unmapped)}")
    print(f"teams: {df['team_id'].n_unique()}   total yards: {df['prod_weight'].sum():,.0f}")
    out = "data/cfb_production_2003.parquet"
