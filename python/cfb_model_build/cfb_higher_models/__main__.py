"""``python -m cfb_model_build.cfb_higher_models <cmd>``."""
...
def main(argv=None) -> int:
    ...
    for name in ("backtest", "fit-pregame", "train-game", "experiments", "hierarchical"):
        ...
        if name == "hierarchical":
            s.add_argument(
                ...
                default="eb",
                choices=("eb", "mcmc"),
                help="eb = closed-form empirical Bayes (default; the MCMC arm is "
                "void under the pre-registration -- see PREREG_hierarchical.md)",
            )
        if name == "experiments":
            s.add_argument(
                "--fresh",
    ...
    if args.cmd == "hierarchical":
        ...
        return run_hier(args.seasons, args.out_dir, method=args.method)
