...
def _build_report(m, args, fig_dir: Path, out: Path) -> ModelReport:
    ...
    if mt == "ep":
            ...
            if make_figs:
                ...
                if class_oof.exists():
                    ...
                    notes.append(
                        ...
                        "(run `python -m cfb_model_build.model_training.loso_oof_extra --targets ep`)."
                    )
        ...
        if period_oof.exists():
            ...
            if nm:
                ...
                notes.append(
                    ...
                    "loso_wp_naive_oof.parquet via `python -m cfb_model_build.model_training.loso_oof_extra "
        ...
        if Path(loso).exists():
            ...
            notes.append("rb_eval LOSO metrics require xrepa_loso.parquet (run `python -m cfb_model_build.rb_eval train`).")
