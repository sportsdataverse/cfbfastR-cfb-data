...
def derive_quarter_aligned(pbp_parquet, oof_parquet):
    ...
    if not (pp.exists() and op.exists()):
        return None
    try:
        import cfb_model_build.model_training.constants as C
    except Exception:
        return None
...
def build_wp_calibration(oof_parquet, fig_dir: Path, out: Path, *, stem: str,
    ...
    try:
        ...
        from cfb_model_build.model_training.validate import calibration_table, weighted_cal_error
    except Exception:
        return []
...
def build_rb_calibration(loso_parquet, fig_dir: Path, out: Path) -> list[str]:
    ...
    try:
        from cfb_model_build.model_training.figures import write_calibration
        from cfb_model_build.rb_eval.validate import calibration_table, weighted_cal_error
    except Exception:
        return []
...
def build_fd_figures(model_path, pbp_parquet, fig_dir: Path, out: Path,
    ...
    try:
        ...
        from cfb_model_build.model_training.fourth_down.constants import FD_FEATURES, FD_LABEL_OFFSET
        ...
        from cfb_model_build.model_training.fourth_down.validate import calibration_fd
    except Exception:
        return []
    booster = xgb.Booster()
