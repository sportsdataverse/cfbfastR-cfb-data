...
def test_off_sr_excludes_st():
    ...
    assert abs(result["OffSR"][0] - 0.50) < 1e-9
...
def test_off_er_explosive_rate():
    ...
    assert abs(result["OffER"][0] - 0.25) < 1e-9
...
def test_avg_eqppp():
    ...
    assert abs(result["AvgEqPPP"][0] - 0.45) < 1e-9
...
def test_iso_ppp_only_successful():
    ...
    assert abs(result["IsoPPP"][0] - 0.85) < 1e-9
...
def test_opp_rate():
    ...
    assert abs(result["OppRate"][0] - 2 / 3) < 1e-9
...
def test_opp_eff():
    ...
    assert abs(result["OppEff"][0] - 0.5) < 1e-9
...
def test_opp_ppd():
    ...
    assert abs(result["OppPPD"][0] - 3.5) < 1e-9
...
def test_exp_to_formula():
    ...
    assert abs(result["ExpTO"][0] - (0.22 * 2 + 0.49 * 1)) < 1e-9
...
def test_punt_return_eqppp_equals_punt_eqppp():
    ...
    assert result["PuntReturnEqPPP"][0] == result["PuntEqPPP"][0]
