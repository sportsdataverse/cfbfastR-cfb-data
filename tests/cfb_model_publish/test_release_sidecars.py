...
def test_sidecars_carry_a_name_and_a_timestamp(tmp_path):
    ...
    assert expected == "sportsdataverse.cfb.load_cfb_ratings()"
...
def test_an_artifact_tag_with_no_loader_names_its_producer():
    assert PKG_FUNCTION.get("espn_cfb_model_artifacts", _PRODUCER) == _PRODUCER
    assert _PRODUCER.endswith("artifacts.py")
