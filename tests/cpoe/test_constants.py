...
def test_constants_imports():
    from cfb_model_build.cpoe import constants  # noqa: F401
...
def test_throw_depth_buckets_structure():
    from cfb_model_build.cpoe.constants import THROW_DEPTH_BUCKETS
...
def test_xgb_params_is_dict():
    from cfb_model_build.cpoe.constants import XGB_PARAMS
...
def test_xgb_nrounds_positive_int():
    from cfb_model_build.cpoe.constants import XGB_NROUNDS
    assert isinstance(XGB_NROUNDS, int)
    assert XGB_NROUNDS > 0


def test_pass_play_types_is_set():
    from cfb_model_build.cpoe.constants import PASS_PLAY_TYPES
