...
def test_down1_50pct_is_successful():
    df = pl.DataFrame([_play("Rush", 1, 10, 5)])  # 5 >= 0.5*10
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_successful"][0] is True or out["play_successful"][0] == True  # noqa: E712


def test_down1_below_50pct_is_not_successful():
    df = pl.DataFrame([_play("Rush", 1, 10, 4)])  # 4 < 5
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_successful"][0] == False  # noqa: E712


def test_down2_70pct_is_successful():
    df = pl.DataFrame([_play("Rush", 2, 10, 7)])  # 7 >= 0.7*10
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_successful"][0] == True  # noqa: E712


def test_down3_not_successful_by_default():
    # 3rd down: default is False regardless of yards (not in np.select conditions)
    df = pl.DataFrame([_play("Rush", 3, 5, 100)])
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_successful"][0] == False  # noqa: E712


def test_explosive_15_yards():
    df = pl.DataFrame([_play("Rush", 1, 10, 15)])
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_explosive"][0] == True  # noqa: E712


def test_bad_type_not_explosive():
    df = pl.DataFrame([_play("Interception", 1, 10, 15)])
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_explosive"][0] == False  # noqa: E712


def test_st_type_not_successful():
    df = pl.DataFrame([_play("Kickoff", 0, 0, 60)])
    out = add_play_features(df, [], ST_TYPES, BAD_TYPES)
    assert out["play_successful"][0] == False  # noqa: E712
...
def test_eqppp_computed_for_off_play():
    ...
    assert abs(out["EqPPP"][0] - (ep[30] - ep[20])) < 1e-9
