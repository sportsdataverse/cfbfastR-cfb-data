...
KNOWN_UNPAIRED: dict[str, str] = {
    # R builds all ten advanced-box datasets inside its single numbered stage
    # espn_cfb_04_adv_box_creation.R, from one pass over `advBoxScore` -- eight
    # sections via .ADV_MAP and two sdv-py expansion sections via .ADV_EXTRA.
    # Python splits them into ten REGISTRY datasets (shims 20-29). This is a
    # decomposition, NOT a gap: the same ten outputs exist on both sides.
    **{
        key: (
            "R writes it inside espn_cfb_04_adv_box_creation.R (verified: "
            f"{where} maps the advBoxScore section to this dataset) rather than "
            "as its own numbered stage. Python's twin shim is numbered in the "
            "20-29 block; the orchestrator at 04 builds all ten."
        )
        for key, where in {
            "adv_team": ".ADV_MAP",
            "adv_passing": ".ADV_MAP",
            "adv_rushing": ".ADV_MAP",
            "adv_receiving": ".ADV_MAP",
            "adv_defensive": ".ADV_MAP",
            "adv_turnover": ".ADV_MAP",
            "adv_drives": ".ADV_MAP",
            "adv_situational": ".ADV_MAP",
            "adv_defensive_players": ".ADV_EXTRA",
            "adv_specialists": ".ADV_EXTRA",
        }.items()
    },
}
